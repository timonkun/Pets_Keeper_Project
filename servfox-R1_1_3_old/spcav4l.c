...
int
convertframe(unsigned char *dst, unsigned char *src, int width, int height,
	     int formatIn, int size)
{
	int jpegsize = 0;
	switch (formatIn) {
...
int v4lGrab(struct vdIn *vd)
{
	...
	if (vd->grabMethod) {
		...
		temps = ms_time();
		jpegsize =
		    convertframe(vd->ptframe[vd->frame_cour] +
				 sizeof(struct frame_t),
				 vd->pFramebuffer +
		...
		headerframe->deltatimes =
		    (int)(headerframe->seqtimes - timecourant);
	...
	/************************************/
		...
		jpegsize =
		    convertframe(vd->ptframe[vd->frame_cour] +
				 sizeof(struct frame_t), vd->pFramebuffer,
				 vd->hdrwidth, vd->hdrheight, vd->formatIn,
				 vd->framesizeIn);
...
***/
	...
	if (debug)
		printf
		    ("VIDIOCGPICT brightnes=%d hue=%d color=%d contrast=%d whiteness=%d"
...
***/
	if (debug)
		printf
		    ("VIDIOCSPICT brightnes=%d hue=%d color=%d contrast=%d whiteness=%d"
		     "depth=%d palette=%d\n", vd->videopict.brightness,
		     vd->videopict.hue, vd->videopict.colour,
		     vd->videopict.contrast, vd->videopict.whiteness,
		     vd->videopict.depth, vd->videopict.palette);
...
static void spcaSetQuality(int fd, struct video_param *videoparam,
...
static int init_v4l(struct vdIn *vd)
{
	...
	if (ioctl(vd->fd, VIDIOCGCHAN, &vd->videochan) == -1) {
		...
		if (vd->videochan.name) {
			...
			if (debug)
				printf
				    ("Bridge not found not a spca5xx Webcam Probing the hardware !!\n");
			vd->cameratype = UNOW;
		}
	}
...
*/
  ...
  ***/
	if (vd->grabMethod) {
		...
		if (debug)
			printf
			    ("VIDIOCGMBUF size %d  frames %d  offets[0]=%d offsets[1]=%d\n",
			     vd->videombuf.size, vd->videombuf.frames,
			     vd->videombuf.offsets[0],
			     vd->videombuf.offsets[1]);
		vd->pFramebuffer =
		    (unsigned char *)mmap(0, vd->videombuf.size,
		...
		vd->pFramebuffer =
		    (unsigned char *)realloc(vd->pFramebuffer,
					     (size_t) vd->framesizeIn);
		if (debug)
...
{				/* probe palette and set a default one for unknow cams */
	int pal[] =
	    { VIDEO_PALETTE_JPEG, VIDEO_PALETTE_YUV420P, VIDEO_PALETTE_RGB24,
...
{				/* probe size and set a default one for unknow cams */
	int size[] =
	    { 640, 480, 384, 288, 352, 288, 320, 240, 192, 144, 176, 144, 160,
	   120 };
	...
	while ((size[i] >= minw) && i < 13) {
		...
		if (ioctl(vd->fd, VIDIOCSWIN, &win) < 0) {
			if (debug)
				printf
				    ("VIDIOCSWIN reject width %d  height %d \n",
				     win.width, win.height);
		} else {
			vd->sizeothers = vd->sizeothers | masq;
			if (debug)
				printf
				    ("Available Resolutions width %d  heigth %d \n",
				     win.width, win.height);
			if (defaut) {
...
static int checkpalette(struct vdIn *vd)
{
	...
	if (!(vd->palette & needpalette)) {
		...
		if ((masq & vd->palette) > 1) {
			...
			while (!((needpalette = needpalette >> 1) & vd->palette)
			       && needpalette) ;
...
static int check_palettesize(struct vdIn *vd)
{
	...
	if (!(vd->sizeothers & needsize)) {
		...
		if ((masq & vd->sizeothers) > 1) {
			/* check lower masq upper size */
			while (!((needsize = needsize >> 1) & vd->sizeothers)
			       && needsize) ;
...
static void spcaPrintParam(int fd, struct video_param *videoparam)
{
	...
	***/
	printf("quality %d autoexpo %d Timeframe %d \n",
	       videoparam->quality, videoparam->autobright,
	       videoparam->time_interval);
}
